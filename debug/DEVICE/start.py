...
class smacInit():
    SENDING_INFO = 0
    TIME_SYNC = False
    RESET_BUTTON = const(35)
    MSG_COUNT_PREV = 0
    DEVICE_DATA_SEND_INTERVAL = const(60)
    TOPIC_MSG_CNT = {}
    WIFI_STAT_PREV = False
    ...
    def on_message(self, topic, message, protocol, *args):
        ...
        try:
            ...
            if( topic not in  [config.ID_DEVICE, "#"]):
                ...
                if self.TOPIC_MSG_CNT.get(id_topic) != None:
                    self.TOPIC_MSG_CNT[id_topic] += 1
                else:
                    self.TOPIC_MSG_CNT[id_topic] = 1
            ...
            if (frm != config.ID_DEVICE):
                ...
                elif cmd == smac_keys["CMD_REQ_SEND_INFO"]:
                ...
                elif cmd == smac_keys["CMD_ADD_TOPIC"]:
                ...
                elif cmd == smac_keys["CMD_REMOVE_TOPIC"]:
                    id_topic = data.get(smac_keys["ID_TOPIC"])
                    id_device = data.get(smac_keys["ID_DEVICE"])
                    passkey = data.get(smac_keys["PASSKEY"])
                    self.delete_topic(frm, id_topic, id_device, passkey, msg_id)

                elif cmd == smac_keys["CMD_UPDATE_WIFI_CONFIG"]:
                ...
                elif cmd == smac_keys["CMD_UPDATE_SOFTWARE"]:
                ...
                elif cmd == smac_keys["CMD_UPDATE_INTERVAL_ONLINE"]:
                ...
                elif cmd == smac_keys["CMD_UPDATE_NAME_DEVICE"]:
                ...
                elif cmd == smac_keys["CMD_UPDATE_NAME_PROPERTY"]:
                ...
                elif cmd == smac_keys["CMD_ADD_ACTION"]:
                    add_action(data, frm)

                elif cmd == smac_keys["CMD_ADD_TRIGGER"]:
                    add_trigger(data, frm)

                elif cmd == smac_keys["CMD_REMOVE_ACTION"]:
                    remove_action(data, frm)

                elif cmd == smac_keys["CMD_REMOVE_TRIGGER"]:
                    remove_trigger(data, frm)

                elif cmd == smac_keys["CMD_TRIGGER_CONTEXT"]:
    ...
    async def interval(self):
        ...
        while 1:
            ...
            if( COUNTER % 5 ) == 0:
                ...
                print("checking wifi_status")
                w =  wifi_client.wlan.isconnected()
                if w != self.WIFI_STAT_PREV:
                    self.WIFI_STAT_PREV = w
                    self.network_callback(w)
            ...
            if(COUNTER % 10) == 0:
                ...
                print(self.TOPIC_MSG_CNT)
                for id_topic in self.TOPIC_MSG_CNT.keys():
                    if self.TOPIC_MSG_CNT[id_topic] >= 50:
                        client.block_topic(id_topic)
                        config.update_config_variable(key="blocked_topic", value=id_topic, arr_op="ADD")
                    del self.TOPIC_MSG_CNT[id_topic]
    ...
    def network_callback(self, status):
        print("wifi connection changed: ", status)
