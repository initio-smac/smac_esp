var DATA = {};
    ...
    function showLoader(text) {
        document.getElementById("id_loader_wrapper").style.display = "block";
        //document.getElementById("id_loader").innerHTML = text;
        document.getElementById("id_loader").style.display = "block";
    }

    function hideLoader() {
        document.getElementById("id_loader_wrapper").style.display = "none";
        //document.getElementById("id_loader").innerHTML = "";
        document.getElementById("id_loader").style.display = "bnone";
    }


   function rest_call(url, method="GET", data={}, refresh_on_success=true, show_loader=false) {
        //document.getElementById("id_loader").style.display = "block";
        if(show_loader) {
          showLoader()
        }
    ...
    function changeSpeed(ele, op) {
      ...
      //console.log(value)
        ...
        rest_call(url="/change_property", method="GET", data=p1, refresh_on_success=false, show_loader=true)
    }
              ...
              if(this.status == 200) {
                  ...
                  try {
                        ...
                        for(j=0; j< props.length; j++) {
                            ...
                            }else if(props[j].type_property == "1") {
                              prop_data += "<div> " + props[j].name_property + " : <button onclick='changeSpeed(this, 1)' data-prop='" +JSON.stringify(props[j])+ "'> + </button>" + props[j].value + "   <button onclick='changeSpeed(this, 0)' data-prop='" +JSON.stringify(props[j])+ "'> - </button> </div> <br> <br>"
                            }
                        }
                        document.getElementById("id_props").innerHTML +=  prop_data;
                  } catch(err) {
